...
- 전체 데이터: `136,988`행
- 학습/검증/홀드아웃: `89,604 / 19,586 / 27,798`
- Validation Macro F1: `0.9918`
- Holdout Accuracy: `0.9829`
- Holdout Balanced Accuracy: `0.9814`
- Holdout Macro F1: `0.9819`
- Holdout Weighted F1: `0.9830`
- Bagging estimators: `600`
- 추가학습: 전세가율·법적 신호 반례 `16,000`건 + 신원/대항력/체납/이중계약 현장패턴 `18,000`건 + 사용자 자연어 입력 변형 `9,000`건 + 구어체 텍스트-only 입력 `8,000`건 + 계좌·계약금·건축물 텍스트-only 입력 `6,000`건 + 전대차·임차권등기·미등기·가계약금 텍스트-only 입력 `3,000`건 + 전입세대·선순위·경매공매·소유자변경 텍스트-only 입력 `3,000`건 + 대지권·주소호수·무상거주확인서·법인권한·말소접수 텍스트-only 입력 `3,000`건 + 비대면 신원·중개사 자격·보증서 캡처·다운계약 텍스트-only 입력 `2,500`건 + 잔금 최신 등기부·전입 공백·전세대출 브로커·가족 계좌 송금 텍스트-only 입력 `2,000`건 추가
...
- API 예측: `POST /api/predict` 200, 안전 최신 등기부·즉시전입 예시 `안전 15.5점`, 오래된 등기부 재열람 거부 예시 `주의 66.0점`, 주말 전입 공백 예시 `위험 74.0점`, 전세대출 브로커 명의대여 예시 `위험 76.0점`, 가족 계좌 송금 예시 `위험 74.0점`, 고위험 기본 예시 `위험 93.0점`
- 자동 테스트: `15 passed in 19.06s`
- Playwright 로드: 페이지 제목 `부동산 계약 위험 탐지`, 주요 입력 폼, `위험도 분석` 버튼 렌더링, 클릭 후 결과 `위험 · 92.8점` 확인
- 참고 데스크톱/모바일 캡처:
...
flowchart LR
  ...
  K["사용자 자연어 입력 변형"] --> D
  L["구어체 텍스트-only 입력"] --> D
  M["계좌·계약금·건축물 텍스트-only 입력"] --> D
  N["전대차·권원 텍스트-only 입력"] --> D
  O["전입세대·경매공매 텍스트-only 입력"] --> D
  P["공적장부·권한서류 불일치 텍스트-only 입력"] --> D
  Q["비대면 신원·중개서류 텍스트-only 입력"] --> D
  R["잔금 최신서류·전입공백 텍스트-only 입력"] --> D
...
- 텍스트: 계약서 특약, 사용자 상황, 판례 기반 위험 키워드, 보호요건 완료 안전 신호, 구어체 안전 확인, 명의 불일치, 대리권 서류 미확인, 계좌 명의 불일치, 가족 계좌 송금 요구, 계약금 선입금 압박, 전입신고 지연, 당일 근저당, 잔금 직전 최신 등기부 재열람 거부, 잔금 후 전입·점유 공백, 전세대출 브로커 명의대여, 신탁원부 미확인, 보증보험 불가, 압류 축소 설명, 불법증축·쪼개기, 전대차, 임차권등기명령, 미등기, 사용승인 전, 가계약금 환불불가, 전입세대열람 미제공, 확정일자 부여현황 미제공, 선순위 임차보증금 총액 불명, 경매개시결정, 공매 예고, 소유자 변경, 매매·전세 동시진행, 잔금 당일 대출, 대지권 미등기, 등기부·건축물대장·계약서 호수 불일치, 무상거주확인서, 전세권 설정 거부, 법인 권한서류 미확인, 근저당 말소접수 전 입금, 비대면 신분증 사진-only, 폐업 중개사무소, 공제증서 미제공, 보증서 캡처 조회거부, 다운계약 차액 현금, 체납, 이중계약 치명 신호
