...
- 전체 데이터: `123,488`행
- 학습/검증/홀드아웃: `81,099 / 17,885 / 24,504`
- Validation Macro F1: `0.9950`
- Holdout Accuracy: `0.9837`
- Holdout Balanced Accuracy: `0.9830`
- Holdout Macro F1: `0.9826`
- Holdout Weighted F1: `0.9837`
- Bagging estimators: `560`
- 추가학습: 전세가율·법적 신호 반례 `16,000`건 + 신원/대항력/체납/이중계약 현장패턴 `18,000`건 + 사용자 자연어 입력 변형 `9,000`건 + 구어체 텍스트-only 입력 `8,000`건 + 계좌·계약금·건축물 텍스트-only 입력 `6,000`건 추가
...
- API 예측: `POST /api/predict` 200, 안전 전세 예시 `안전 15.5점`, 고위험 기본 예시 `위험 89.8점`, 계좌 명의 불일치 예시 `위험 74.0점`
- 자동 테스트: `10 passed in 7.67s`
...
- 텍스트: 계약서 특약, 사용자 상황, 판례 기반 위험 키워드, 보호요건 완료 안전 신호, 구어체 안전 확인, 명의 불일치, 대리권 서류 미확인, 계좌 명의 불일치, 계약금 선입금 압박, 전입신고 지연, 당일 근저당, 신탁원부 미확인, 보증보험 불가, 압류 축소 설명, 불법증축·쪼개기, 체납, 이중계약 치명 신호
