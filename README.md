...
- 전체 데이터: `126,488`행
- 학습/검증/홀드아웃: `83,148 / 18,155 / 25,185`
- Validation Macro F1: `0.9880`
- Holdout Accuracy: `0.9813`
- Holdout Balanced Accuracy: `0.9807`
- Holdout Macro F1: `0.9803`
- Holdout Weighted F1: `0.9814`
- Bagging estimators: `580`
- 추가학습: 전세가율·법적 신호 반례 `16,000`건 + 신원/대항력/체납/이중계약 현장패턴 `18,000`건 + 사용자 자연어 입력 변형 `9,000`건 + 구어체 텍스트-only 입력 `8,000`건 + 계좌·계약금·건축물 텍스트-only 입력 `6,000`건 + 전대차·임차권등기·미등기·가계약금 텍스트-only 입력 `3,000`건 추가
...
- API 예측: `POST /api/predict` 200, 안전 전세 예시 `안전 15.5점`, 고위험 기본 예시 `위험 92.8점`, 전대차 텍스트-only 예시 `위험 70.0점`
- 자동 테스트: `11 passed in 10.31s`
- Playwright 로드: 페이지 제목 `부동산 계약 위험 탐지`, 주요 입력 폼, `위험도 분석` 버튼, 결과 화면 `위험 · 90.3점`, 모바일 폭 렌더링 확인
- 참고 데스크톱/모바일 캡처:
...
- 텍스트: 계약서 특약, 사용자 상황, 판례 기반 위험 키워드, 보호요건 완료 안전 신호, 구어체 안전 확인, 명의 불일치, 대리권 서류 미확인, 계좌 명의 불일치, 계약금 선입금 압박, 전입신고 지연, 당일 근저당, 신탁원부 미확인, 보증보험 불가, 압류 축소 설명, 불법증축·쪼개기, 전대차, 임차권등기명령, 미등기, 사용승인 전, 가계약금 환불불가, 체납, 이중계약 치명 신호
