...
- 전체 데이터: `129,488`행
- 학습/검증/홀드아웃: `84,630 / 18,749 / 26,109`
- Validation Macro F1: `0.9912`
- Holdout Accuracy: `0.9754`
- Holdout Balanced Accuracy: `0.9739`
- Holdout Macro F1: `0.9745`
- Holdout Weighted F1: `0.9755`
- Bagging estimators: `600`
- 추가학습: 전세가율·법적 신호 반례 `16,000`건 + 신원/대항력/체납/이중계약 현장패턴 `18,000`건 + 사용자 자연어 입력 변형 `9,000`건 + 구어체 텍스트-only 입력 `8,000`건 + 계좌·계약금·건축물 텍스트-only 입력 `6,000`건 + 전대차·임차권등기·미등기·가계약금 텍스트-only 입력 `3,000`건 + 전입세대·선순위·경매공매·소유자변경 텍스트-only 입력 `3,000`건 추가
...
- API 예측: `POST /api/predict` 200, 안전 전세 예시 `안전 15.5점`, 고위험 기본 예시 `위험 93.0점`, 경매개시결정 텍스트-only 예시 `위험 78.0점`, 선순위 보증금 총액 불명 예시 `위험 72.0점`
- 자동 테스트: `12 passed in 11.99s`
...
- 텍스트: 계약서 특약, 사용자 상황, 판례 기반 위험 키워드, 보호요건 완료 안전 신호, 구어체 안전 확인, 명의 불일치, 대리권 서류 미확인, 계좌 명의 불일치, 계약금 선입금 압박, 전입신고 지연, 당일 근저당, 신탁원부 미확인, 보증보험 불가, 압류 축소 설명, 불법증축·쪼개기, 전대차, 임차권등기명령, 미등기, 사용승인 전, 가계약금 환불불가, 전입세대열람 미제공, 확정일자 부여현황 미제공, 선순위 임차보증금 총액 불명, 경매개시결정, 공매 예고, 소유자 변경, 매매·전세 동시진행, 잔금 당일 대출, 체납, 이중계약 치명 신호
