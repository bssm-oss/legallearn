...
- 전체 데이터: `132,488`행
- 학습/검증/홀드아웃: `86,166 / 19,289 / 27,033`
- Validation Macro F1: `0.9926`
- Holdout Accuracy: `0.9807`
- Holdout Balanced Accuracy: `0.9791`
- Holdout Macro F1: `0.9797`
- Holdout Weighted F1: `0.9807`
- Bagging estimators: `600`
- 추가학습: 전세가율·법적 신호 반례 `16,000`건 + 신원/대항력/체납/이중계약 현장패턴 `18,000`건 + 사용자 자연어 입력 변형 `9,000`건 + 구어체 텍스트-only 입력 `8,000`건 + 계좌·계약금·건축물 텍스트-only 입력 `6,000`건 + 전대차·임차권등기·미등기·가계약금 텍스트-only 입력 `3,000`건 + 전입세대·선순위·경매공매·소유자변경 텍스트-only 입력 `3,000`건 + 대지권·주소호수·무상거주확인서·법인권한·말소접수 텍스트-only 입력 `3,000`건 추가
...
- API 예측: `POST /api/predict` 200, 안전 전세 예시 `안전 15.5점`, 고위험 기본 예시 `위험 93.0점`, 무상거주확인서 텍스트-only 예시 `위험 76.0점`, 호수 불일치 텍스트-only 예시 `위험 70.0점`
- 자동 테스트: `13 passed in 13.81s`
- Playwright 로드: 페이지 제목 `부동산 계약 위험 탐지`, 주요 입력 폼, `위험도 분석` 버튼 렌더링, 클릭 후 결과 `위험 · 93.0점` 확인
- 참고 데스크톱/모바일 캡처:
